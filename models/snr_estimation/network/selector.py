...
def network_selector(network_type, inp, n_outp, **kwargs):
	if network_type == 'MHANetV3':
		from .attention import MHANetV3
		network = MHANetV3(
			inp=inp,
			n_outp=n_outp,
		...
		from .attention import MHANetV2
		network = MHANetV2(
			inp=inp,
			n_outp=n_outp,
			d_model=kwargs['d_model'],
